...
public class AvoidSuperClassCheckTest {
  ...
  public void checkWithJarDependenciesInClassPath() throws Exception {
	 ...
	 * .onFile("src/test/files/AvoidSuperClassCheck.java") .withCheck(new
	 * AvoidSuperClassRule()) .withClassPath(CLASSPATH_JAR) .verifyIssues();
	 */
    
    //JavaCheckVerifier.verify("src/test/files/AvoidSuperClassCheck.java", new AvoidSuperClassRule());
  }
}
